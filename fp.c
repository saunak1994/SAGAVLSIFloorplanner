...
extern anneal(NET * net_arr, MODULE * module_arr, int module_num) ;
extern genetic(NET * net_arr, MODULE * module_arr, int module_num, int soln_area);
...
int crossoverTransition(NET * net_arr, MODULE * module_arr, int i1, int i2){
	int initcost = solution_cost(solution, net_arr, module_arr);
	int type ;
	FPTREE * temp ;
	if((nodes[i1]->operator == 'V' || nodes[i1]->operator == 'H')&&(nodes[i2]->operator == 'V' || nodes[i2]->operator == 'H'))
	{
		int i ;
		for(i = i1; ; i--)
		{
			if(nodes[i] == NULL || nodes[i]->operator == 0){break;}
			else{
				if(nodes[i]->operator == 'H'){nodes[i]->operator = 'V';}
				else{if(nodes[i]->operator == 'V'){nodes[i]->operator = 'H';}}	
			}
		}
		for(i = i2; ; i++)
		{
	...
	if(nodes[i2]->operator == 0)
	{
		...
		if( operator_count < operant_count && (i2 == 0 || nodes[i2 - 1]->operator != nodes[i2]->operator )){//legal normalized polish 			expression
		}
		else{
			nodes[i2 + 1] = nodes[i2];
			nodes[i2] = temp ;
...
int operatorChange(NET * net_arr,MODULE * module_arr, int i1)
{
	int initcost = solution_cost(solution, net_arr, module_arr);
	if(nodes[i1]->operator == 'V' || nodes[i1]->operator == 'H')				//if node is operator
	{
		if(nodes[i1]->operator == 'H'){nodes[i1]->operator = 'V';}			//Change operator
		else{if(nodes[i1]->operator == 'V'){nodes[i1]->operator = 'H';}}
		
		if(i1 == 0 || nodes[i1 - 1]->operator != nodes[i1]->operator )			//If change causes illegal NPE
		{
			if(nodes[i1]->operator == 'H'){nodes[i1]->operator = 'V';}		//Change back
	   		else{if(nodes[i1]->operator == 'V'){nodes[i1]->operator = 'H';}}
		}
	...
	solution = list2tree(nodes);
	iter_update_tree(solution);
	iter_update_module(0, 0, solution, module_arr);						//Update tree
	if(solution->width <= 100 && solution->height <= 100){
		return solution_cost(solution, net_arr, module_arr) - initcost ;		//return difference if change is not extreme
	}
	else return 0;
...
int operandSwap(NET * net_arr,MODULE * module_arr, int i1)
{
	int initcost = solution_cost(solution, net_arr, module_arr);
	FPTREE * temp;
	if(nodes[i1]->operator==0 && nodes[i1+1]->operator==0)					//if node and next node are operands 
	{							
		temp = nodes[i1] ;								//Swap them								
		nodes[i1] = nodes[i1 + 1] ;
		nodes[i1 + 1] = temp ;
	}
...
int main(int argc, char ** argv){
	...
	soln_area= solution->width*solution->height;
	printf("area: %d x %d = %d\n", solution->width, solution->height,soln_area) ;
	iter_update_module(0, 0, solution, module_arr);
	cost=solution_cost(solution, net_arr, module_arr);
	printf("COST: %d\n",cost);
	...
	/*print_list(nodes);
	printf("\n\n\n");
	transition(net_arr, module_arr, 196);
	cost=solution_cost(solution, net_arr, module_arr);
	printf("COST: %d\n",cost);
	print_list(nodes);*/
	//traverse_list(list2tree(nodes));
	//print_net(net_arr);
	anneal( net_arr, module_arr,TOTAL_MODULES);
	//genetic(net_arr, module_arr,TOTAL_MODULES,soln_area);
	return 0;
}
/*
